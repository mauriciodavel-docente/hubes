...
async function run() {
  ...
  try {
    ...
    const tests = [
      { name: 'documento', upload: true, url: '/api/documentos', body: { titulo: 'Doc smoke', tipo: 'Ofício', categoria: 'Teste', descricao: 'Criado pelo smoke test', data: '2026-08-01', numeroDocumento: 'DOC-SMOKE-001', nivelAcesso: 'Público', status: 'Ativo', assinado: 'true' }, file: tmpFile },
      ...
      // Purchase payload now excludes centroCusto and valor; backend generates them.
    ...
    for (const test of tests) {
      try {
        if (test.upload) {
          // Try fetch FormData first
          try {
            const res = await uploadWithFetch(token, test.url, test.body, test.file);
            const success = res.status >= 200 && res.status < 300;
            results.push({ test: test.name, status: success ? 'created' : 'failed', http: res.status, response: res.body });
            ...
            try {
              const parts = [];
              for (const k of Object.keys(test.body)) { parts.push('-F'); parts.push(`${k}=${test.body[k]}`); }
              parts.push('-F'); parts.push(`arquivo=@${test.file}`);
